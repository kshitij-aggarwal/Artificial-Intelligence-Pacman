    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                ...
                self.beliefs[pos] = self.beliefs[pos] * emissionModel[dist]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for part in self.particles:
                distance = util.manhattanDistance(pacmanPosition, part)
                blf[part] += emissionModel[distance]
            if all(i == 0 for i in blf.values()):
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(blf) for i in self.particles]
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            distr = self.getPositionDistribution(self.setGhostPosition(gameState, pos))
            tpart.append(util.sample(distr))
        self.particles = tpart
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    par *= emissionModels[i][distance]
            possible[particle] +=  par
        ...
        if not any(possible.values()):
            ...
            for _ in range(0, self.numParticles):
                temp.append(util.sample(possible))
            self.particles = temp
    ...
    def elapseTime(self, gameState):
        ...
        """
        newparticles = []
        for currentpart in self.particles:
            newpart = list(currentpart) # A list of ghost positions

            # now loop through and update each entry in newParticle...
            for index in xrange(len(newpart)):
                newpart[index] = util.sample(
                    getPositionDistributionForGhost(
                        setGhostPositions(gameState, currentpart),
                        index,
                        self.ghostAgents[index])
                    )

            newparticles.append(tuple(newpart))
